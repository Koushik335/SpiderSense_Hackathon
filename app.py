...
# ---------------------------------------------------------------------------
# 10/10 Tactical Cyberpunk & Chromatic Comic UI Engine
# ---------------------------------------------------------------------------
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Bangers&family=Space+Grotesk:wght@500;600;700;800&family=JetBrains+Mono:wght@400;600;700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
...
.stApp {
    ...
    background-image: 
        radial-gradient(circle at 10% 0%, rgba(230, 36, 41, 0.18) 0%, transparent 45%),
        radial-gradient(circle at 90% 5%, rgba(0, 240, 255, 0.15) 0%, transparent 45%),
...
/* Tactical HUD Header */
.tactical-navbar {
    background: linear-gradient(135deg, rgba(13, 19, 36, 0.96) 0%, rgba(6, 10, 22, 0.98) 100%);
    border: 2px solid #1E293B;
    border-top: 4px solid #E62429;
    border-radius: 18px;
    padding: 22px 30px;
    margin-bottom: 24px;
    box-shadow: 0 20px 45px rgba(0,0,0,0.85), inset 0 1px 0 rgba(255,255,255,0.1);
    display: flex;
...
.control-hub-box {
    background: linear-gradient(135deg, rgba(13, 19, 34, 0.95) 0%, rgba(8, 12, 24, 0.98) 100%);
...
/* Tactical Glassmorphic Content Card */
.sp-card {
...
.verdict-hero-card {
    background: linear-gradient(135deg, rgba(230, 36, 41, 0.14) 0%, rgba(10, 15, 30, 0.9) 100%);
...
div.stButton > button {
    ...
    box-shadow: 0 6px 25px rgba(230, 36, 41, 0.5) !important;
    transition: all 0.25s ease !important;
    width: 100%;
}
div.stButton > button:hover {
    transform: translateY(-2px) !important;
    box-shadow: 0 10px 35px rgba(230, 36, 41, 0.8) !important;
}
</style>
...
with c1:
    ticker = st.selectbox("Target Equity", TICKERS, help="NSE listed equities universe")
with c2:
...
# ---------------------------------------------------------------------------
# Distinct Clean Multi-Page Interface
# ---------------------------------------------------------------------------
...
if not st.session_state.has_run:
    st.info("💡 Adjust target parameters above and click **'DISPATCH MULTI-AGENT SWARM'** to start autonomous research.")
else:
    ...
    # =========================================================================
    # PAGE 1: MASTER VERDICT & SYNTHESIS
    # =========================================================================
    with tab1:
        st.markdown('<div class="sp-card">', unsafe_allow_html=True)
        st.markdown(f'<div class="sp-card-header">Master Synthesis Verdict // {m_ticker} <span>{m_profile}</span></div>', unsafe_allow_html=True)
        ...
        <div class="verdict-hero-card">
            <div style="font-family:'JetBrains Mono'; font-size:0.82rem; color:#FFD700; text-transform:uppercase; font-weight:800; letter-spacing:0.08em;">
                TARGET INVESTOR: {m_profile.upper()}
            </div>
            <div class="verdict-action" style="color:{action_col};">
                {synthesis['action']}
            </div>
            <div style="font-size:1.1rem; color:#E2E8F0; line-height:1.65; max-width:920px; margin: 14px auto 0 auto;">
                {synthesis['reasoning']}
            </div>
        </div>
        """, unsafe_allow_html=True)

        st.progress(synthesis["confidence"], text=f"Synthesis Confidence Level: {synthesis['confidence']:.0%}")

        st.markdown('<div class="sp-card-header" style="margin-top:28px;">Transparent Reasoning Trace</div>', unsafe_allow_html=True)
        for out in outputs:
            st.markdown(f"""
    ...
    with tab2:
        st.markdown('<div class="sp-card">', unsafe_allow_html=True)
        st.markdown('<div class="sp-card-header">Specialized Parallel Agents // Real-Time Telemetry</div>', unsafe_allow_html=True)
            ...
            with col:
                ...
                <div class="agent-node" style="border-left: 4px solid {status_col};">
                    <div>
                        ...
                        <div style="font-family:'Space Grotesk'; font-size:1.6rem; font-weight:800; color:{status_col}; margin:14px 0 4px 0;">
                            {o.label}
                        </div>
                        <div style="font-family:'JetBrains Mono'; font-size:0.78rem; color:#94A3B8;">
                            CONFIDENCE // {o.confidence:.0%}
                        </div>
                        <div style="font-size:0.86rem; color:#CBD5E1; line-height:1.55; margin:14px 0; background:rgba(15,23,42,0.6); padding:12px; border-radius:8px;">
                            {o.reasoning}
                        </div>
                    </div>
                    <div style="border-top:1px solid #1E293B; padding-top:12px; font-family:'JetBrains Mono'; font-size:0.74rem; color:#64748B; display:flex; justify-content:space-between;">
    ...
    with tab3:
        st.markdown('<div class="sp-card">', unsafe_allow_html=True)
        st.markdown('<div class="sp-card-header">Behavioral Personalization Proof // Identical Feed across Risk Profiles</div>', unsafe_allow_html=True)
            ...
            with c:
                ...
                <div class="agent-node" style="border: 2px solid {'#00F0FF' if is_active else '#1E293B'}; background:{'rgba(0,240,255,0.05)' if is_active else '#080D1A'};">
                    <div>
                        ...
                        <div style="font-family:'Space Grotesk'; font-size:1.35rem; font-weight:800; color:{action_col}; margin:12px 0 8px 0;">
                            {syn2['action']}
                        </div>
                        <div style="font-size:0.86rem; color:#CBD5E1; line-height:1.55; margin:10px 0;">
                            {syn2['reasoning']}
                        </div>
                    </div>
                    <div style="border-top:1px solid #1E293B; padding-top:10px; font-family:'JetBrains Mono'; font-size:0.75rem; color:#94A3B8;">
        ...
        if not synthesis["citations"]:
            ...
            for c in synthesis["citations"]:
                ...
                <div class="citation-box">
                    <div style="font-family:'Space Grotesk'; font-size:1.1rem; font-weight:700; color:#FFD700;">
    ...
    with tab5:
        ...
        st.markdown('<div class="sp-card-header" style="margin-top:28px;">Historical Audit Trail (Persistent Across Sessions)</div>', unsafe_allow_html=True)
